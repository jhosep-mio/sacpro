...
import { type subcategoriasValues } from '../../../shared/Interfaces'
import { DeleteItems } from '../../../shared/DeleteItems'
import { LoadingSmall } from '../../../shared/LoadingSmall'

export const ListarSubcategorias = (): JSX.Element => {
  ...
  const getAllSubcategorias = async (): Promise<void> => {
  ...
  const totalPosts = subcategorias.length

  const filterDate = (): never[] => {
    return subcategorias.slice(indexOfFirstPost, indexOfLastPost)
  }
  ...
  const preguntar = (id: number): void => {
    DeleteItems({
      ruta: 'deleteSubcategorias',
      id,
      token,
      getData: getAllSubcategorias,
      totalPosts,
      cantidadRegistros,
      paginaActual,
      setpaginaActual
  ...
  useEffect(() => {
    setTitle('Listado de Sub Categorías')
  }, [])
        ...
        <div className="w-full flex flex-col lg:flex-row items-center justify-between gap-4">
          <button className="bg-secondary-100/50 hover:bg-secondary-100  w-full md:w-fit flex items-center  gap-2 py-2 px-4 rounded-lg hover:text-white transition-colors">
            ...
            <button
              ...
              onClick={() => {
                !loadingComponents && getAllSubcategorias()
              }}
            >
              {!loadingComponents
                ? (
